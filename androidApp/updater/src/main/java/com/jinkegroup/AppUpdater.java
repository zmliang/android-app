...
import com.jinkegroup.utils.JavaUtils;
import com.jinkegroup.widget.CheckDialogFragment;
import com.jinkegroup.widget.ProgressDialogFragment;
...
public class AppUpdater {
    ...
    private CheckDialogFragment updateDialog;
                ...
                updateService.setUpdateListener(new UpdateListener() {
                    ...
                    public void onDownloading(Integer... values) {
                        if (values[1] == 0) {
                            ...
                            customViews.setTextViewText(R.id.notify_progress_size, format.replace("%1f", JavaUtils.formatFloat2String(values[1]/fCount, 2)).replace("%2f", JavaUtils.formatFloat2String(fileSize/fCount, 2)));
                    ...
                    public void onFoundNewVersion(UpdateModel appInfo) {
                        ...
                        updateDialog = new CheckDialogFragment();
                        ...
                        updateDialog.setOnButtonClickListener(new CheckDialogFragment.OnClickBtnListener() {
                            @Override
                            public void clicked(CheckDialogFragment dialog, int which) {
                                switch (which) {
                                    case CheckDialogFragment.BTN_OK:
                                        dialog.dismiss();
                                        fileDownload(downloadUrl);
                                        break;
                                    case CheckDialogFragment.BTN_CANCEL:
                                        release();
                                        break;
                                }
                            }
    ...
    private void release() {
        if (context!=null && serviceConn!=null)
            context.unbindService(serviceConn);
        updateService = null;
        serviceConn = null;
        notifyBuilder = null;
