...
public class SystemPackageTools {
     ...
     */
    public static String getApkVersion(File apkFile) {
            ...
            if (apkFile.exists()) {
                ...
                if (!xmlResult.isEmpty()) {
                    ...
                    if (name.equalsIgnoreCase("Manifest")) {
                        pakVersion = parser.getAttributeValue(null, "versionCode");
                    }
    ...
    public static int getApkVersion(Context context, File apkFile, boolean deleteOnFailure) {
        if (!apkFile.isFile())
            return -1;

        final PackageManager pm = context.getPackageManager();
        String fullPath = apkFile.getAbsolutePath();
    ...
    public static int getApkVersion(Context context, File apkFile) {
        return getApkVersion(context, apkFile, false);
    }


    /*
     * return -1 if package not exists
     * return versionCode if package installed
     */
    ...
    public static String getSignature(Context context, File apkFile) {
        if (!apkFile.isFile())
            return "";

        final PackageManager pm = context.getPackageManager();
        String fullPath = apkFile.getAbsolutePath();
        PackageInfo info = pm.getPackageArchiveInfo(fullPath, PackageManager.GET_SIGNATURES);
